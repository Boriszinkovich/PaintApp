...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (SMPageControlTapBehaviorJump == self.tapBehavior) {
        ...
        {
            if ([self.theDelegate respondsToSelector:@selector(pageControl:didReceiveTapAndWillMoveToPageAtIndex:)])
            {
                [self.theDelegate pageControl:self didReceiveTapAndWillMoveToPageAtIndex:tappedIndicatorIndex];
            }
            [self setCurrentPage:tappedIndicatorIndex sendEvent:YES canDefer:YES];
            if ([self.theDelegate respondsToSelector:@selector(pageControl:didReceiveTapAndDidMoveToPageAtIndex:)])
            {
                [self.theDelegate pageControl:self didReceiveTapAndDidMoveToPageAtIndex:self.currentPage];
            }
            return;
        }
    }
    ...
    if (point.x < middle)
    {
        if ([self.theDelegate respondsToSelector:@selector(pageControl:didReceiveTapAndWillMoveToPageAtIndex:)])
        {
            [self.theDelegate pageControl:self didReceiveTapAndWillMoveToPageAtIndex:self.currentPage - 1];
        }
